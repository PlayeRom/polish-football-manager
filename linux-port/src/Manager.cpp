...
void Manager::runManager()
{
    ...
    wchar_t pp1, pp2, pp_usta, pp_usta2, tn;
    int dalej = 0;
    int k = 0, kk, i = 0, tryb = 0;
    int sumaO = 0, sumaP = 0, sumaN = 0, lata;
    bool isNewGame = false;
    bool isOneLoopForBkgColor = false;
    ...
    do {
        ...
        if (!isOneLoopForBkgColor) {
            isOneLoopForBkgColor = true;
            pp1 = 'X'; // znak ktorego nie ma w menu, aby program wykonał jedną pętlę dla zamalowania tła na czarno
        }
        else {
            pp1 = pInput->getKeyBoardPressed();
        }
        ...
        switch (pp1) {
            case 'N': {
                ...
                //wcout << endl << endl << L"Proszę czekać..."; // nie ma na co czekac, hula szybko
                ...
                messages[0] = 1; // PREZES: Panie %ls, witam pana w nowym miejscu pracy i mam nadzieję iż spełni pan nasze oczekiwania.
                messages[1] = 1;
                stringForMessage[0] = clubRef.managerSurname;
                messages[2] = 7; // ASYSTENT: Radzę panu ustalić kilka meczy kontrolnych. Pozwoli to panu lepiej przygotować się do rozgrywek ligowych.
            ...
            case 'W': { // wczytaj grę
                ...
                do {
                    ...
                    if (!clubRef.isRivalSet) {
                        ...
                        if (clubRef.rivalData[0] == 1 ||
                            clubRef.rivalData[0] == 10 ||
                            clubRef.rivalData[0] == 12 ||
                            clubRef.rivalData[0] == 13 ||
                            clubRef.rivalData[0] == 16
                        ) {
                            clubRef.rivalData[2] = 1;
                        }
                        if (clubRef.rivalData[0] == 2 || clubRef.rivalData[0] == 4 || clubRef.rivalData[0] == 11) {
                            clubRef.rivalData[2] = 12;
                        }
                        if (clubRef.rivalData[0] == 3 || clubRef.rivalData[0] == 9) {
                            clubRef.rivalData[2] = 6;
                        }
                        if (clubRef.rivalData[0] == 5) {
                            clubRef.rivalData[2] = 13;
                        }
                        if (clubRef.rivalData[0] == 6) {
                            clubRef.rivalData[2] = 2;
                        }
                        if (clubRef.rivalData[0] == 7) {
                            clubRef.rivalData[2] = 10;
                        }
                        if (clubRef.rivalData[0] == 8 || clubRef.rivalData[0] == 14) {
                            clubRef.rivalData[2] = 3;
                        }
                        if (clubRef.rivalData[0] == 15) {
                            clubRef.rivalData[2] = 14;
                        }
                        clubRef.rivalInst[0] = (rand() % 4) + 1; //podania zawsze los
                        sumaN = rand() % 3;
                        clubRef.rivalInst[1] = (sumaN == 0)
                            ? 3
                            : 1; //nigdy delikatne

                        sumaN = rand() % 4;
                        clubRef.rivalInst[2] = (sumaN == 0)
                            ? 0
                            : 1; //pressing 3/4 że tak

                        if (clubRef.rivalData[2] == 1 || clubRef.rivalData[2] == 6 || clubRef.rivalData[2] == 3 || clubRef.rivalData[2] == 10) {
                            clubRef.rivalInst[3] = 1; //pułapki tak
                        }
                        else if (clubRef.rivalData[2] == 12 || clubRef.rivalData[2] == 14) {
                            clubRef.rivalInst[3] = rand() % 2;
                        }
                        else {
                            clubRef.rivalInst[3] = 0; //end pułapki ofsajdowe
                        }

                        if (clubRef.rivalData[2] == 2 || clubRef.rivalData[2] == 13) {
                            clubRef.rivalInst[4] = 1;
                        }
                        else if (clubRef.rivalData[2] == 3 || clubRef.rivalData[2] == 10 || clubRef.rivalData[2] == 14) {
                            clubRef.rivalInst[4] = 0;
                        }
                        else {
                            clubRef.rivalInst[4] = rand() % 2; //end gra z kontry
                        }

                        if (clubRef.rivalData[2] == 2 || clubRef.rivalData[2] == 13) {
                            sumaN = rand() % 3; //(rand() % 3);
                            clubRef.rivalInst[5] = (sumaN == 0)
                                    ? 1
                                    : 2; //2/3 że obronne, atak wcale
                        }
                        else if (clubRef.rivalData[2] == 3 || clubRef.rivalData[2] == 10 || clubRef.rivalData[2] == 14) {
                            sumaN = rand() % 3; //(rand() % 3);
                            clubRef.rivalInst[5] = (sumaN == 0)
                                    ? 1
                                    : 3; //2/3 że atak, obronne wcale
                        }
                        else if (clubRef.rivalData[2] == 6 || clubRef.rivalData[2] == 12) {
                            sumaN = (rand() % 4);
                            if (sumaN == 0) {
                                clubRef.rivalInst[5] = 2;
                            }
                            else if (sumaN == 1) {
                                clubRef.rivalInst[5] = 3;
                            }
                            else {
                                clubRef.rivalInst[5] = 1; //2/4 że normalne
                            }
                        }
                            ...
                            clubRef.rivalInst[5] = (sumaN == 0)
                                    ? 3
                                    : 1; //2/3 że normalne, obronne wcale
                        }
                    ...
                    info();
                    dalej = 0;
                    pp2 = displayMainMenu(messages, tactic.getTeamSettings());
                    pColors->textcolor(GREEN);
                    switch (pp2) {
                        case 'C': {//***************** kontynuuj ********************
                            if (!clubRef.isMatch && clubRef.clubId > 0) {
                                pColors->textcolor(LIGHTGRAY);
                                wcout << endl << L"Proszę czekać..." << endl;
                                clubRef.isBlockTransferFunds = 0;
                                //*********** lista transferowa ****************************
                                pFootballers->changeTransferList();
                                //********************************************************
                                if (messages[0] == 1) { //czy wogóle jest jakaś wiadomość
                                    for (i = 1; i < 10; i++) {
                                        const SNews& news = pNews->getManagerNewsByNumber(messages[i]);
                                        if (news.num > 0) {
                                            pNews->addManagerMessage(news, clubRef, stringForMessage[i - 1].c_str());
                                ...
                                while (!k) {
                                    ...
                                    if ((clubRef.month == 4 || clubRef.month == 6 || clubRef.month == 9 || clubRef.month == 11) && clubRef.day == 31) {
                                        clubRef.day = 1;
                                        clubRef.month++;
                                    }
                                            ...
                                            if (messages[i] == 0 && numer1 == 0) {
                                                ...
                                                if (clubRef.month == 4 ||
                                                    ...
                                                    if (clubId == clubRef.clubId) {
                                                        messages[i + 2] = clubId + 60;
                                                        stringForMessage[i + 1] = clubRef.managerSurname;
                                                        clubRef.managerStats[2]++;
                                                        clubRef.managerStats[3] += (clubRef.managerStats[2] * 10);
                                                    }
                                                    else {
                                                        messages[i + 2] = clubId + 44;
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    float placa = 0;
                                    float premia = 0;
                                    for (i = clubRef.weekNumber - 1; i < 28; i += 7) { //sprawdzamy dany dzien
                                        ...
                                        //if (clubRef.training[i]==9) sumaN++;//wolne
                                    }
                                    ...
                                    i = rand() % 12; // losuje czy odjąć 1/12
                                    if (lata == 0 && i == 0) {
                                        clubRef.treBOPN--;
                                        int footballerId = (rand() % playerTeamSize) + 1; // lozuj ktory pilkarz
                                        ...
                                        for (size_t index = 0; index < playerTeamSize; index++) {
                                            ...
                                            if (footballer.data[0] == footballerId) { //--
                                                i = (rand() % 4) + 3; // losujemy ktora umiejetnosc spada
                                                footballer.data[i]--;
                                                if (footballer.data[i] < 1) {
                                                    footballer.data[i] = 1;
                                                    clubRef.treBOPN++;
                                                }
                                    ...
                                    i = rand() % 6; //losuje czy dodać trening BOPN (1/6 szans)
                                    if (lata > 0 && i == 0) {
                                    ...
                                    for (size_t index = 0; index < pFootballers->getSizePlayerTeam(); index++) {
                                        ...
                                        if (footballer.data[19] == 1) {
                                            ...
                                            for (i = 1; i < 10; i++) {
                                                if (messages[i] == 0) {
                                                    messages[i] = 6; // LEKARZ: %ls doznał kontuzji. Za około %d dni będzie do dyspozycji.
                                                    stringForMessage[i - 1] = footballer.surname;
                                                    numbersForMessage[i - 1] = (rand() % 83) + 8; // za ile dni wyleczy kontuzje
                                                    footballer.data[15] = numbersForMessage[i - 1];
                                        ...
                                        if (footballer.data[15] > 0) {
                                            ...
                                            if (footballer.data[15] == 0 || footballer.data[15] == 7) {
                                                ...
                                                for (int i = 1; i < MAX_MESSAGES; i++) {
                                                    if (messages[i] == 0) {
                                                        if (footballer.data[15] == 0) {
                                                            messages[i] = 4; // LEKARZ: %ls całkowicie wyleczył kontuzję, zaczął trenować i może grać.
                                                        }
                                                        else if (footballer.data[15] == 7) {
                                                            messages[i] = 5; // LEKARZ: %ls częściowo wyleczył kontuzję i może grać, ale jeszcze nie trenuje.
                                                        }
                                        ...
                                        if (footballer.data[18] == 0) { //kontrakt
                                            sumaP = footballer.data[0];
                                            sumaN++;
                                            if (sumaN >= 2) {
                                                sumaP -= sumaN - 1;
                                            }
                                    ...
                                    //******************** komunikaty kibiców ******************
                                    kk = (rand() % 6);
                                    if (kk == 0 && clubRef.playerGoals != clubRef.rivalGoals) {
                                        ...
                                        for (i = 1; i < 10; i++) {
                                            if (messages[i] == 0) {
                                                messages[i] = kk;
                                                stringForMessage[i - 1] = clubRef.isNick ? clubRef.nick : clubRef.managerSurname;
                                    ...
                                    if (clubRef.isRiot) {
                                        ...
                                        for (i = 1; i < 10; i++) {
                                            if (messages[i] == 0) {
                                                messages[i] = 44;
                                                stringForMessage[i - 1] = pClub->getClubName(clubRef.clubId - 1);
                                    ...
                                    if (clubRef.month == 6 && clubRef.day == 23) {
                                        int clubId = pTable->getClubNumberInPosition(1); // wez klub na pierwszym miejscu
                                        ...
                                        for (i = 1; i < 10; i++) {
                                            if (messages[i] == 0) {
                                                messages[i] = 78;
                                                stringForMessage[i - 1] = pClub->getClubName(clubId - 1);
                                                break;
                                            }
                                        }
                                    }
                            ...
                            else if (clubRef.isMatch && clubRef.clubId > 0) {
                            ...
                            else { //clubRef.clubId==0 manager bezrobotny
                                info();
                        ...
                        case 'A': { // taktyka
                            if (clubRef.clubId > 0) {
                                do {
                                    info();
                                    tactic.drawTeamSetting(clubRef.teamSetting);
                                    tactic.drawChart(clubRef.teamSetting, clubRef.clubId, pFootballers->getPlayersTeam());
                                    teamComposition.draw(pFootballers->getPlayersTeam(), clubRef.teamSetting, 11, clubRef.clubId);

                                    pp_usta = pInput->getKeyBoardPressed();
                                    switch (pp_usta) {
                                        case 'U': {
                                            bool loop = false;
                                            while (!loop) {
                                                ...
                                                wcout << L"Obecne ustawienie: " << tactic.getTeamSetting(clubRef.teamSetting - 1);
                                                pColors->textcolor(LIGHTGRAY);
                                                ...
                                                for (int i = 0; i < 14; i++) {
                                                    ...
                                                    wcout << i + 1 << L". " << tactic.getTeamSetting(i);
                                                }
                                        ...
                                        case 'R': {
                                            ...
                                            teamComposition.draw(pFootballers->getPlayersTeam(), clubRef.teamSetting, 16, clubRef.clubId);
                                            pColors->textcolor(LIGHTGRAY);
                                        ...
                                        case 'I': {
                                            ...
                                            do {
                                                ...
                                                pp_usta2 = pInput->getKeyBoardPressed();
                                                switch (pp_usta2) {
                                                    case _KEY_UP:
                                                        if (--belka == 0) {
                                                            belka = 6;
                        ...
                        case 'S': { // skład
                            if (clubRef.clubId > 0) {
                                ...
                                do {
                                    ...
                                    pColors->textcolor(LIGHTGRAY); //i=0;
                                    if (sort == 0) {
                                        if (dalej == 0) {
                                            tryb = 20;
                                        }
                                    ...
                                    teamComposition.draw(pFootballers->getPlayersTeam(), clubRef.teamSetting, tryb, clubRef.clubId, false, sort);
                                    ...
                                    switch (pp_usta) {
                                        ...
                                        case 'D': {
                                            if (sort == 0) {
                                                dalej = (dalej == 0 && playersCount > 20) ? 1 : 0;
                                            }
                                            break;
                                        }
                                        case 'S': {
                                            ...
                                            switch (pp_usta2) {
                                                case 'B':
                                                    ...
                                                    break;
                                                case 'O':
                                                    sort = 4; //kk którą daną grcza
                                                    break;
                                                case 'P':
                                                    sort = 5; //kk którą daną grcza
                                                    break;
                                                case 'N':
                                                    sort = 6; //kk którą daną grcza
                                                    break;
                                                case 'M'://morale
                                                    sort = 7; //kk którą daną grcza
                                                    break;
                                                case 'F'://forma
                                                    sort = 9;
                                                    break;
                                            ...
                                            for (size_t index = 0; index < pFootballers->getSizePlayerTeam(); index++) {
                                                ...
                                                if (footballer.data[0] == footballerId) {
                                                    do {
                                                        ...
                                                        wstring morale = teamComposition.getMorale(footballer.data[7]);
                                                        wprintf(
                                                        ...
                                                        switch (pp_usta2) {
                                                            ...
                                                            case 'T': {
                                                                if (footballer.data[17] == 0) {
                                                                    ...
                                                                    //scanf(L"%f", &footballer.finances[3]);
                                                            ...
                                                            case 'K': {
                                                                ...
                                                                sumaO = footballer.data[3] + footballer.data[4] + footballer.data[5] + footballer.data[6];
                                                                if (footballer.data[2] == PLAYERS_POS_B && sumaO > 30) weight -= 2;
                                                                if (sumaO >= 30) weight--;
                                                                if (sumaO >= 50) weight -= 2;
                                                                if (sumaO < 20) weight++;
                                                                if (sumaO < 10) weight += 2;
                                    ...
                                    switch (trenningMenu) {
                                        ...
                                        case L'7': {
                                            ...
                                            switch (tranningPlanMenu) {
                                                case '1': {//B,O,P,N
                                                    for (i = day - 1; i < 28; i += 7) {
                                                        k++;
                                                        if (k == 1) clubRef.training[i] = TRENNING_B;
                                                        if (k == 2) clubRef.training[i] = TRENNING_O;
                                                        if (k == 3) clubRef.training[i] = TRENNING_P;
                                                        if (k == 4) clubRef.training[i] = TRENNING_N;
                                                    }
                                                    pClub->save();
                                                    break;
                                                }
                                                case  '3': { //wolne
                                                    ...
                                                    for (i = day - 1; i < 28; i += 7) {
                                                        k++;
                                                        if (k == 1) clubRef.training[i] = TRENNING_HOLIDAY;
                                                        if (k == 2) clubRef.training[i] = TRENNING_EMPTY;
                                                        if (k == 3) clubRef.training[i] = TRENNING_EMPTY;
                                                        if (k == 4) clubRef.training[i] = TRENNING_EMPTY;
                                                    }
                                                    pClub->save();
                                                    break;
                                                }
                                        ...
                                        case 'D': { // kto ma trenować B,O,P,N
                                            ...
                                            do {
                                                ...
                                                teamComposition.draw(pFootballers->getPlayersTeam(), clubRef.teamSetting, tryb, clubRef.clubId, true);
                        ...
                        case 'P': { // najbliższy przeciwnik
                            if (clubRef.clubId > 0) {
                                do {
                                    ...
                                    tactic.drawTeamSetting(clubRef.rivalData[2], false);//clubRef.rivalData[0]);

                                    tactic.drawChart(clubRef.rivalData[2], clubRef.rivalData[0], pFootballers->getRivals(), true);
                                    tactic.drawChart(clubRef.teamSetting, clubRef.clubId, pFootballers->getPlayersTeam());

                                    teamComposition.draw(pFootballers->getRivals(), clubRef.rivalData[2], 11, clubRef.rivalData[0]);
                                    ...
                                    switch (pp_usta) {
                                        case 'R': {
                                            ...
                                            teamComposition.draw(pFootballers->getRivals(), clubRef.rivalData[2], 16, clubRef.rivalData[0]);
                        ...
                        case 'K': { //kalendarz
                            if (clubRef.clubId > 0) {
                                ...
                                do {
                                    if (dalej == 0) { // runda jesienna, 1-15
                                        roundNum = 0;
                                        tryb = MAX_CLUBS - 1;
                                    }
                                    ...
                                    pColors->textcolor(WHITE);
                                    wcout << endl << L"KALENDARZ - " << (dalej == 0 ? L"runda jesienna" : L"runda wosenna");

                                    pColors->textcolor(LIGHTGRAY);
                                    if (clubRef.controlMatchesAmount <= 0 && dalej == 0) {
                                        ...
                                        case 'M': { // mecze kontrolne
                                            if (clubRef.month == 7) { // 7 miesiac
                                                ...
                                                if (!canPlayControlMatch) {
                                                    ...
                                                    for (i = 0; i < MAX_CLUBS; i++) {
                                                        for (k = 0; k < 19; k++) {
                                                            round.kol1[i][k] = pClub->getClubName(round.clubNumbers[i] - 1)[k];
                                                        }
                                                    }
                                ...
                                do {
                                    ...
                                    while (i != tryb) {
                                        ...
                                        for (size_t index = 0; index < pFootballers->getSizeTransfers(); index++) {
                                            ...
                                            if (footballer.data[0] == i && footballer.data[22] != clubRef.clubId) {
                                                    ...
                                                    if (footballer.data[0] == footballerId) {
                                                        ...
                                                        if (weight >= 6) {
                                                            ...
                                                            if (tn == L't' || tn == L'\n') {
                                                                ...
                                                                // TODO: mysle ze tu trzeba usunac zawodnika z listy transferowej bo bedzie zdublowany?
                        ...
                        case 'D': {// zarzad klubu
                            if (clubRef.clubId > 0) {
                                do {
                                    ...
                                    if (clubRef.finances[12] <= 500000 && clubRef.finances[11] <= 0)        messages[9] = 35;
                                    else if (clubRef.finances[12] <= 500000 && clubRef.finances[11] > 0)    messages[9] = 36;
                                    else if (clubRef.finances[12] <= 1000000 && clubRef.finances[11] <= 0)  messages[9] = 37;
                                    else if (clubRef.finances[12] <= 1000000 && clubRef.finances[11] > 0)   messages[9] = 38;
                                    else if (clubRef.finances[12] <= 2000000 && clubRef.finances[11] <= 0)  messages[9] = 39;
                                    else if (clubRef.finances[12] <= 2000000 && clubRef.finances[11] > 0)   messages[9] = 40;
                                    else if (clubRef.finances[12] > 2000000 && clubRef.finances[11] <= 0)   messages[9] = 41;
                                    else if (clubRef.finances[12] > 2000000 && clubRef.finances[11] > 0)    messages[9] = 42;

                                    for (i = 8; i < 10; i++) {
                                        wcout << endl << endl;
                                        const SNews& news = pNews->getManagerNewsByNumber(messages[i]);
                                        if (news.num > 0) {
                                            wprintf(news.message, stringForMessage[i - 1].c_str(), numbersForMessage[i - 1]);
                                        }
                                    }
                                    messages[8] = 0;
                                    messages[9] = 0;
                                    ...
                                    switch (pp_usta) {
                                        case 'T': {
                                            if (clubRef.finances[12] <= 0)                                              kk = 1; // bez szans
                                            else if (clubRef.finances[12] > 0 && clubRef.finances[12] <= 500000)        kk = (rand() % 6); // 1/6 szansy
                                            else if (clubRef.finances[12] > 500000 && clubRef.finances[12] <= 1000000)  kk = (rand() % 5); // 1/5 szansy
                                            else if (clubRef.finances[12] > 1000000 && clubRef.finances[12] <= 2000000) kk = (rand() % 4); // 1/4 szansy
                                            else if (clubRef.finances[12] > 2000000)                                    kk = (rand() % 3); // 1/3 szansy

                                            if (kk == 0 && !clubRef.isBlockTransferFunds) {
                                                pColors->textcolor(GREEN);
                                                wcout << endl << endl <<
                            ...
                            if (messages[0] == 1) { //czy wogóle jest jakaś wiadomość
                                for (i = 1; i < 10; i++) {
                                    ...
                                    if (news.num > 0) {
                                        ...
                                        pNews->addManagerMessage(news, clubRef, stringForMessage[i - 1].c_str(), numbersForMessage[i - 1]);
                                    }
                                }
                                pNews->saveManagerMessages();
                                pInput->getKeyBoardPressed();
                ...
                while (pp2 != 'Q');
                break; //dla pp1 W Wczytaj grę
            }
        }//dla switch pp1
    }//dla do
    while (pp1 != 'Q');

    /*if (!clubRef.isNotAllowedTeamPlayer && clubRef.isMatch == 1) {
        //pInput->clrscr();
        pColors->textcolor(LIGHTGREEN);
        //wcout << endl << L"Aby rozegrać mecz uruchom plik Mecz.exe" << endl;
        //if (isGoToMatch) {
            //execl("mecz.exe", NULL, NULL);
...
/**
 * // Znajdz "%s" i wstaw "l" czyli = "%ls" aby móc formatować w wchar_t
 * // Juz nie potrzebne bo plik zawiera od razu %ls
 * @param news
 */
/*void prepareNewsStringWideChar(News &news)
{
    for (int l = 0; l < MAX_NEWS_LENGTH - 1; l++) {
        if (news.newss[l] == L'%' && news.newss[l + 1] == L's') {
            wchar_t tmp[MAX_NEWS_LENGTH];
            for (int j = 0, k = l + 1; j < MAX_NEWS_LENGTH - 1 && k < MAX_NEWS_LENGTH - 1; j++, k++) {
                tmp[j] = news.newss[k];
                if (tmp[j] == 0) {
                    break;
                }
            ...
            for (int j = l + 2, k = 0; j < MAX_NEWS_LENGTH - 2 && k < MAX_NEWS_LENGTH; j++, k++) {
                news.newss[j] = tmp[k];
            }
            l += 2;
        }
            ...
            for (int i = 1; i < MAX_MESSAGES; i++) {
                if (messages[i] == 0) {
                    ...
                    if (AnaO < -20 && clubRef.inst[4] == 0) {
                        messages[i + counter] = 20;
                        counter++;
                    }
                    if (PnaP < -10 && clubRef.inst[0] != 4) {
                        messages[i + counter] = 21;
                        counter++;
                    }
                    if (OnaA < 0 && clubRef.inst[3] == 0) {
                        messages[i + counter] = 22;
                        counter++;
                    }
                    if (OnaA > 20 && clubRef.inst[3] == 1) {
                        messages[i + counter] = 23;
                        counter++;
                    }
                    if (PnaP > 10 && clubRef.inst[0] != 2) {
                        messages[i + counter] = 24;
                        counter++;
                    }
                    if (AnaO > 0 && clubRef.inst[4] == 1) {
                        messages[i + counter] = 25;
                        counter++;
                    }
                    if (clubRef.inst[2] == 0) {
                        messages[i + counter] = 26;
                        counter++;
                    }
                    if (clubRef.inst[2] == 1) {
                        messages[i + counter] = 27;
                        counter++;
                    }
                    if (clubRef.inst[1] != 3) {
                        messages[i + counter] = 28;
                        counter++;
                    }
                    if (counter > 0) {
                       messages[0] = 1;
